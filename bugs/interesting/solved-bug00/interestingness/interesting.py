...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            SOURCE = os.path.join(temp_dir, "HIPProg.hip")
            shutil.copy2(ORIG_SOURCE, SOURCE)

            TEST_BIN_ASAN = os.path.join(temp_dir, "test_bin_asan")
            TEST_BIN_UBSAN = os.path.join(temp_dir, "test_bin_ubsan")
            TEST_BIN_MSAN = os.path.join(temp_dir, "test_bin_msan")
            TEST_BIN_CPU_REF = os.path.join(temp_dir, "test_bin_cpu_ref")
            ...
            CMD_BASE = [
                ...
                "-Werror=array-bounds",
                #"-Werror=zero-length-array", here we error but should be discussed why we have this on!
