...
def main():
    SOURCE = "HIPProg.hip"
    DRIVER_GPU_O = os.path.join(BUG_DIR, "driver_gpu.o")
    DRIVER_CPU_O = os.path.join(BUG_DIR, "driver_cpu.o")
    ...
    try:
        CMD_BASE = [
            "hipcc", DRIVER_GPU_O, "-x", "hip", SOURCE, "-I", PROJECT_INCLUDE,
            "-Werror=uninitialized", "-Werror=c99-designator", "-Wno-c++20-designator", "-Wl,--allow-multiple-definition",
        ...
        CPU_BASE = [
            "clang++", DRIVER_CPU_O, "-x", "c++", SOURCE,
            "-I", HIP_CPU_INCLUDE,
            "-I", PROJECT_INCLUDE,
            "-Wno-c++11-narrowing", "-Wno-unused-value", "-Wno-constant-conversion", "-Wl,--allow-multiple-definition",
        ...
        if not os.path.exists(DRIVER_GPU_O) or not os.path.exists(DRIVER_CPU_O):
            log_decision("REJECTED", "Pre-compiled driver objects missing (.o files)")
            sys.exit(1)
